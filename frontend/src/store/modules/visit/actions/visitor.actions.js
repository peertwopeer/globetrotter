import * as GUESTTypes from "../mutationTypes/GUEST.mutationTypes";
...
export default {
  handleCheckIn: (context, payload) =>
    HTTP.post(
      ...
      swal(
        `${payload.GUESTName} checked-in ${
          payload.notifyHost ? `and notified ${payload.hostName}` : ""
        }`
      );
      context.commit(GUESTTypes.CHECK_IN_VISIT, payload.id);
      if (payload.notifyHost)
        context.commit(GUESTTypes.NOTIFIED_HOST, payload.id);
    }),
  ...
  selfRegister: (context, payload) =>
    HTTP.post("/api/visit/self-register", payload, {
      ...
      .then((response) => {
        ...
        context.commit(GUESTTypes.ADD_TO_VISITS, visitData);
      })
  ...
  handleCollect: (context, visitId) =>
    context.dispatch("submitVisitAction", {
      type: GUESTTypes.MARK_COLLECTED,
      endPoint: "collect-GUEST",
      visitId,
    }),

  notifyHost: (context, visitId) =>
    context.dispatch("submitVisitAction", {
      type: GUESTTypes.NOTIFIED_HOST,
  ...
  boloPass: (context, visitId) =>
    context.dispatch("submitVisitAction", {
      type: GUESTTypes.BOLO_STATUS_PASSED,
  ...
  boloFail: (context, visitId) =>
    context.dispatch("submitVisitAction", {
      type: GUESTTypes.BOLO_STATUS_FAILED,
  ...
  checkOut: (context, visitId) =>
    context.dispatch("submitVisitAction", {
      type: GUESTTypes.MARK_AS_LEFT,
      endPoint: "check-out-GUEST",
      visitId,
    }),

  removeVisit: (context, visitId) =>
    context.dispatch("submitVisitAction", {
      type: GUESTTypes.REMOVE_VISIT,
