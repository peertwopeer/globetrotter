...
const GUESTS_QR_CODES_DIR = "GUESTs/qrcodes/";
const GUESTS_IMAGE_DIR = "GUESTs/profile-pics/";

const GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL =
  queueMessage.GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL;
const NOTIFY_HOST_EMAIL = queueMessage.NOTIFY_HOST_EMAIL;
const BOLO_PASSED_HOST_EMAIL = queueMessage.BOLO_PASSED_HOST_EMAIL;
const GUEST_SLOT_CONFIRMED_EMAIL = queueMessage.GUEST_SLOT_CONFIRMED_EMAIL;
...
module.exports = {
  confirmVisit: async function (req, res) {
    ...
    // upload GUEST image if we have one
    if (req.body.GUESTImage) {
      try {
        const imageBuffer = Buffer.from(
          req.body.GUESTImage.replace(/^data:image\/\w+;base64,/, ""),
          "base64"
        );
        const imageType = req.body.GUESTImage.split(";")[0].split("/")[1];
        const image_name = Date.now() + "-" + Math.floor(Math.random() * 1000);
        const imagePath = GUESTS_IMAGE_DIR + `${image_name}.${imageType}`;
        await aws
    ...
    QRCode.toDataURL(token, async function (err, base64) {
      try {
        ...
        const QRpath = GUESTS_QR_CODES_DIR + `${QR_name}.${QRtype}`;
        await aws
            ...
            visitModel
              ...
              .then(async (result) => {
                ...
                let payload = {
                  ...
                  GUESTName: `${visitData.firstName} ${visitData.lastName}`,
                  ...
                  queueType: GUEST_SLOT_CONFIRMED_EMAIL,
                };
                queueMessage
  ...
  selfRegister: async function (req, res) {
    ...
    let payload = {
      ...
      queueType: GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL,
      locationManagerName: locationManager.userProfile.firstName,
      GUESTName: `${visitParams.firstName} ${visitParams.lastName}`,
  ...
  checkInGUEST: async function (req, res) {
    const USER = await clientUserModel.findOne({ userId: req.user.id });
    if (!USER) return res.json({ error: "User is not authorized." });
  ...
  checkOutGUEST: async function (req, res) {
    const USER = await clientUserModel.findOne({ userId: req.user.id });
    if (!USER) return res.json({ error: "User is not authorized." });
