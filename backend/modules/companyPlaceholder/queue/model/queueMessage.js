...
const GUEST_INVITATION_EMAIL_TEMPLATE = 2;
const GUEST_SLOT_CONFIRMED_EMAIL_TEMPLATE = 3;
const GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL_TEMPLATE = 4;
...
const GUEST_INVITATION_EMAIL = "GUEST_INVITATION_EMAIL";
const GUEST_SLOT_CONFIRMED_EMAIL = "GUEST_SLOT_CONFIRMED_EMAIL";
const GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL =
  "GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL";
      ...
      queueMessage
        ...
        .then((result) => {
          result.forEach((queue, key) => {
            ...
            if (payload.queueType == GUEST_INVITATION_EMAIL) {
              sendGUESTInvitationLinkMailTemplate(
                payload,
                queue.id,
                key + 1,
                function (err, result) {
            ...
            if (payload.queueType == GUEST_SLOT_CONFIRMED_EMAIL) {
              sendGUESTConfirmedMailTemplate(
                payload,
                queue.id,
                key + 1,
                function (err, result) {
            ...
            if (payload.queueType == GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL) {
              sendGUESTCheckedInToLocManagerMailTemplate(
                payload,
                queue.id,
                key + 1,
                function (err, result) {
...
function sendGUESTInvitationLinkMailTemplate(payload, queueId, key, callback) {
  emailTemplate
    .findOne({
      attributes: ["template"],
      where: { type: GUEST_INVITATION_EMAIL_TEMPLATE },
    })
    .then(function (result) {
      if (result.template) {
        data = result.template;
        data = data.replace("${greetings}", payload.greetings);
        data = data.replace("${GUESTName}", payload.GUESTName);
        ...
        data = data.replace(
          "${logoUrl}",
          payload.logoUrl ??
            "https://s3-eu-west-1.amazonaws.com/static.companyPlaceholder.io/companyPlaceholder/img/logos/companyPlaceholder-vms-logo-icon_mail.png"
        );
...
function sendGUESTConfirmedMailTemplate(payload, queueId, key, callback) {
  emailTemplate
    .findOne({
      attributes: ["template"],
      where: { type: GUEST_SLOT_CONFIRMED_EMAIL_TEMPLATE },
    })
    .then(function (result) {
      if (result.template) {
        data = result.template;
        ...
        payload.attachments = [
          {
            filename: payload.GUESTName + " " + "QR Code.png",
...
function sendGUESTCheckedInToLocManagerMailTemplate(
  payload,
  queueId,
  key,
  callback
  ...
  emailTemplate
    .findOne({
      attributes: ["template"],
      where: { type: GUEST_CHECKED_IN_TO_LOC_MANAGER_EMAIL_TEMPLATE },
    })
    .then(function (result) {
      if (result.template) {
        data = result.template;
...
function changeToDevEmails(email) {
  if (Array.isArray(email)) {
    ...
    email = email + "@loremine.com";
    return email;
  }
}

function sendMail(id, content, payload, count, subjecttext, callback) {
  // To sent mail to loremine domain when mail is triggered from local
